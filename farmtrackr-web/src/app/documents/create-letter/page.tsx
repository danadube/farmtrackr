                      ...
                      onClick={async () => {
                        try {
                          ...
                          if (headerMatch && headerMatch[1]) {
                            setNewLetterheadHeaderHtml(headerMatch[1].trim())
                          } else {
                            // Fallback: if no markers, use first div as header
                            const firstDivMatch = cleanHtml.match(/<div[^>]*>[\s\S]*?<\/div>/)
                            if (firstDivMatch) {
                              setNewLetterheadHeaderHtml(cleanHtml.split(/<!-- FOOTER SECTION -->/)[0].trim())
                            }
                          }
                          
                          if (footerMatch && footerMatch[1]) {
                            setNewLetterheadFooterHtml(footerMatch[1].trim())
                          }
                        } catch (error) {
