...
export function FarmContactsView({ viewSwitcher }: { viewSwitcher?: ReactNode }) {
  ...
  const uniqueFarms = Array.from(
    new Set(contacts.map((c) => (c.farm ? normalizeFarmName(c.farm) : c.farm)).filter(Boolean))
  ).sort()
  ...
  const filteredContacts = contacts
    ...
    .sort((a, b) => {
      ...
      switch (sortBy) {
        case 'name': {
          const aName = (
            a.organizationName || `${a.lastName || ''}, ${a.firstName || ''}`.trim()
          ).toLowerCase()
          const bName = (
            b.organizationName || `${b.lastName || ''}, ${b.firstName || ''}`.trim()
          ).toLowerCase()
          aValue = aName
          bValue = bName
          break
        ...
        case 'dateCreated':
          aValue =
            a.dateCreated instanceof Date ? a.dateCreated.getTime() : new Date(a.dateCreated).getTime()
          bValue =
            b.dateCreated instanceof Date ? b.dateCreated.getTime() : new Date(b.dateCreated).getTime()
          break
        default:
          return 0
      }
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  ...
                  <p style={{ color: 'rgba(255, 255, 255, 0.9)', fontSize: '16px', margin: '0' }}>
                    Manage contacts connected to farms and farming operations
                  </p>
                </div>
              </div>
                ...
                {uniqueFarms.length > 0 && (
                  ...
                  >
                    ...
                    {uniqueFarms.map((farm) => (
                      <option key={farm as string} value={farm as string}>
                        {farm}
                      </option>
                    ))}
                  </select>
            ...
            <div style={{ padding: '48px', textAlign: 'center' }}>
              ...
              <p style={{ ...text.secondary, marginBottom: '24px' }}>
                {searchQuery
                  ...
                  : 'Get started by adding your first farm contact.'}
              </p>
              {!searchQuery && (
                <Link
                  href="/contacts/new?type=farm"
                ...
                <Link
                  ...
                  style={getButtonPressStyle(
                    `contact-${contact.id}`,
                    {
                      ...
                      padding: '20px 24px',
                      borderBottom:
                        index < filteredContacts.length - 1 ? `1px solid ${colors.border}` : 'none',
                      textDecoration: 'none',
                    },
                    'transparent',
                    colors.cardHover
