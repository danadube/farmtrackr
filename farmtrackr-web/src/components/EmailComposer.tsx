...
export function EmailComposer({
  ...
  useEffect(() => {
    setSelectedTransactionId(initialTransactionId || null)
  }, [initialTransactionId])
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (templateList.length === 0) {
          setTemplates(initialTemplates && initialTemplates.length > 0 ? initialTemplates : DEFAULT_EMAIL_TEMPLATES)
        } else {
          // Merge with defaults (ensure unique IDs, keep remote versions)
          const baseTemplates = initialTemplates && initialTemplates.length > 0 ? initialTemplates : DEFAULT_EMAIL_TEMPLATES
          const defaultTemplateMap = new Map(baseTemplates.map((t) => [t.id, t]))
